...
module ovf_udf_comp (
  ...
  input  wire [`CNT_WIDTH-1:0] TCNT,            // Current counter value
    ...
    if (!preset_n) begin
      ...
      // Check counting direction
        if (!count_up_down) begin 
          ...
          if (TCNT_d == {`CNT_WIDTH{1'b1}} && TCNT == {`CNT_WIDTH{1'b0}})
            TMR_OVF <= 1'b1;
          else 
            TMR_OVF <= TMR_OVF;
          ...
          if (TCNT_d == {`CNT_WIDTH{1'b0}} && TCNT == {`CNT_WIDTH{1'b1}})
            TMR_UDF <= 1'b1;
          else 
            TMR_UDF <= TMR_UDF;
